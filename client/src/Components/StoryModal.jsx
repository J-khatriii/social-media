...
const StoryModal = ({setShowModel, fetchStories}) => {
    ...
    const bgColors = ["#4f4fe5", "#7c3aed", "#db2777", "#e11d48", "#ca8a04", "#0d9488"];
        ...
        if(file){
          if(file.type.startsWith("video")){
            if(file.size > MAX_VIDEO_SIZE * 1024 * 1024){
              toast.error(`Video size should not exceed ${MAX_VIDEO_SIZE} MB.`);
              setMedia(null);
              setPreviewUrl(null);
              return;
            }
            ...
            video.onloadedmetadata = () => {
              ...
              if(video.duration > MAX_VIDEO_DURATION){
                toast.error("video duartion limit exceed");
                setMedia(null);
                setPreviewUrl(null);
              }else{
                setMedia(file);
  ...
  return (
    <div className="fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-lg h-96 items-center justify-center flex relative" style={{backgroundColor: background}}>
          {
            mode === 'text' && (
