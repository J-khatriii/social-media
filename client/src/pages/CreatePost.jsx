...
const CreatePost = () => {
  ...
  const handelSubmit = async () => {
    if(!images.length && !content){
      return toast.error("Please add image or text");
    }
    setLoading(true);
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-600">Share your thuoghts with the world</p>
        </div>
        ...
        <div className="max-w-xl bg-white p-4 sm:p-8 sm:pb-3 rounded-xl shadow-md space-y-4">
          ...
          <div className="flex items-center gap-3">
            <img src={user.profile_photo} alt="user" className="w-12 h-12 rounded-full shadow" />
            <div>
        ...
        {/* imges */}
        {
          images.length > 0 && <div className="flex flex-wrap gap-2 mt-4"> {images.map((image, i) => (
            <div onClick={() => setImages(images.filter((_, index) => index !== i))} className="relative group" key={i}>
