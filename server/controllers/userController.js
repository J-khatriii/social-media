...
export const sendConnectionRequest = async (req, res) => {
    try {
        ...
        if(connectionRequests.length >= 20){
            return res.json({success: false, message: "You have sent more than 20 connections requests in last 24 hours"});
        }

        // check if user if already connected
        ...
        if(!connection){
            ...
            //trigger the event for mail for request inngest
            await inngest.send({
                name: "app/connection-request",
                data: { connectionId: newConnection._id },
            });

            return res.json({success: true, message: "connection requests sent successfully."});
        } else if(connection && connection.status === "accepted"){
            return res.json({success: false, message: "You are already connected to this person."});
        } 
...
export const acceptConnectionRequest = async (req, res) => {
    try {
        ...
        return res.json({success: true, message: "connections accepted sucesfully"});
    } catch (error) {
        console.log(error);
        res.json({success: false, message: error.message});
    }
